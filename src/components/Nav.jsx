    ...
    <header>
      <nav className=" mx-auto max-w-7xl px-2 sm:px-6 lg:px-8  flex justify-between items-center max-container">
            ...
            {navLinks.map((item) => (
              <li key={item.label}>
                <a
                  ...
                  className={
                    item.current
                      ? "underline underline-offset-8 decoration-green-500 text-white px-3 py-2 rounded-md text-sm"
                      : "text-gray-300 hover:underline hover:underline-offset-8 hover:decoration-green-500 hover:text-white px-3 py-2 rounded-md text-sm"
                  }
                >
                  {item.label}
                </a>
        ...
        <div className="flex space-x-3 ">
          ...
          {isSideNavOpen ? (
            <button
              className=" text-red-700 h-[25px] text-4xl sm:hidden 0"
            ...
            <button
              className=" focus:ring-2 focus:out focus:outline-none  focus:ring-green-700 rounded text-white h-[25] text-4xl sm:hidden"
              onClick={toggleSideNav}
            >
        ...
        {isSideNavOpen && (
          <div>
            <div className=" absolute sm:hidden right-0 top-0 h-screen w-2/3 bg-green-800 text-white z-10 py-6 px-2 transform translate-x-0 transition-transform duration-300">
              <button
                className=" focus:ring-2 focus:out focus:outline-none  focus:ring-green-700 rounded transition duration-200 my-2 text-white h-[33px] text-4xl sm:hidden mb-2"
                onClick={toggleSideNav}
              >
                <IonIcon icon={close} />
              </button>
